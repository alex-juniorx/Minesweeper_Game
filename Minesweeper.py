#MINESWEEPER GAME
#Game played on terminal on a 9x9 Board, with 6 Bombs, enumerated neighboring positions and final score (based on revealed positions and elapsed time)
...
def bomb_positioning(board, bombs_position):
    for i in range(6):
...
while introduction == True:
    ...
    initial_board(board)
    print_board(board)
    bomb_positioning(board, bombs_position)
    define_solution(solution)
    
    #7.1 The timer is started and the Introduction is turned off:
    t0 = time.time()
    introduction = False
    ...
    while introduction == False:
        ...
        #6.6 In case the selected location is safe (without a bomb):
            ...
            print_board(board)
            print("")
            if final_score(elapsed_time) >= 750:
                print("______________________________________________________")
                print(" ")
